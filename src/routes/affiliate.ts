 ...
 * Allows a promoter to customize their access code (ref_id).
 *
 * Body: { email: string, new_code: string }
 * Validates format, looks up promoter, updates via FirstPromoter API.
 * FirstPromoter enforces uniqueness server-side.
 */
affiliate.put('/referral-code', async (c) => {
  const body = await c.req.json<{ email?: string; new_code?: string }>();
  const { email, new_code } = body;

  if (!email || !new_code) {
    return c.json({ error: 'Both email and new_code are required' }, 400);
  }

  // Validate format: lowercase alphanumeric + hyphens, 3-30 characters
  const codePattern = /^[a-z0-9][a-z0-9-]{1,28}[a-z0-9]$/;
  if (!codePattern.test(new_code)) {
    return c.json({
      error: 'Code must be 3-30 characters, lowercase letters, numbers, and hyphens only. Cannot start or end with a hyphen.',
    }, 400);
  }

  try {
    // Look up the promoter to confirm ownership
    const promoter = await getPromoterByEmail(email, c.env.FIRSTPROMOTER_API_KEY, c.env.FIRSTPROMOTER_ACCOUNT_ID);
    ...
    // Update the promoter's ref_id via FirstPromoter API
    ...
    if (!updateRes.ok) {
      ...
      console.error('FirstPromoter update error:', updateRes.status, errorBody);

      // FirstPromoter returns 422 if the code is already taken
      if (updateRes.status === 422) {
        return c.json({ error: 'This access code is already taken. Please choose another.' }, 409);
      }
      return c.json({ error: 'Failed to update access code' }, 500);
    }

    const updated = await updateRes.json() as Record<string, unknown>;
    const updatedCode = (updated.default_ref_id as string) || new_code;

    console.log(`Access code updated for ${email}: ${updatedCode}`);

    return c.json({
      ref_id: updatedCode,
      referral_link: `https://quantum.optimisingperformance.com.au?ref=${updatedCode}`,
    });
  } catch (err) {
